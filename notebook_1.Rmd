...
```
```{r}
# commute_time_tgts = commute_time %>% select(tgt_counties)
commute_time_tgts <- commute_time %>% filter(
...
```{r}
#  NOT WORKING
# tot_pop_tgts / commute_time_tgts$Value[match(names(tot_pop_tgts), commute_time_tgts$County)][col(tot_pop_tgts)]
```
...
```
```{r}
heart_attack_tgts <- heart_attack %>% filter(
  County %in% tgt_counties
)
heart_attack_tgts
```
